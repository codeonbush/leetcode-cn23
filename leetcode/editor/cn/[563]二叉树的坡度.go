...
输出：9
 ...
 Related Topics 树 深度优先搜索 二叉树 👍 316 👎 0
...
func findTilt(root *TreeNode) int {
	...
	cur := int(math.Abs(float64(left - right)))
	return findTilt(root.Left) + findTilt(root.Right) + cur
}
...
func findTilt(root *TreeNode) int {
	...
	sum = func(root *TreeNode) int {
		...
		// 后序位置
		res += int(math.Abs(float64(leftSum - rightSum)))
